...
export function LoginScreen({ defaultCredentialsActive, onAuthenticated }: { defaultCredentialsActive: boolean; onAuthenticated: (session: AuthSession) => void }) {
  ...
  return <div className="flex min-h-screen items-center justify-center bg-[#F8F9FC] p-6">
    <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
      <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl bg-[#F2EFFF] text-[#6D4AFF]"><ShieldCheck size={24} /></div>
      ...
      <form onSubmit={submit} className="mt-6 space-y-4">
        <label className="block"><span className="mb-2 block text-sm font-medium text-gray-700">用户名</span><div className="relative"><User size={17} className="absolute left-3 top-3 text-gray-400" /><input autoComplete="username" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full rounded-lg border border-gray-200 py-2.5 pl-10 pr-3 text-sm outline-none focus:ring-2 focus:ring-[#6D4AFF]" /></div></label>
        <label className="block"><span className="mb-2 block text-sm font-medium text-gray-700">密码</span><div className="relative"><LockKeyhole size={17} className="absolute left-3 top-3 text-gray-400" /><input autoComplete="current-password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-lg border border-gray-200 py-2.5 pl-10 pr-3 text-sm outline-none focus:ring-2 focus:ring-[#6D4AFF]" /></div></label>
        {error && <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-600">{error}</div>}
        <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#6D4AFF] py-2.5 text-sm font-semibold text-white hover:bg-purple-700 disabled:opacity-50">{loading && <Loader2 size={16} className="animate-spin" />}登录</button>
      </form>
