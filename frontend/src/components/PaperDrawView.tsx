...
import { DrawHistoryItem, GenealogyPaper, QueryInfo, SearchLanguageMode } from '@/types/research';
...
export default function PaperDrawView({ searchLanguageMode, onSearchLanguageModeChange, readingIds, onSelectPaper, onAddReading }: Props) {
  ...
  const draw = async () => {
    ...
    try {
      const result = await genealogyService.draw({ query: effectiveQuery, ...activeRange, foundationalOnly, languageMode: searchLanguageMode });
  ...
  return <div className="space-y-4">
    <div><div className="flex items-center gap-2 text-sm font-semibold text-[#F97316]"><Dice5 size={18} />研究工作区</div><h1 className="mt-1 text-2xl font-bold text-gray-900">基础理论论文抽取</h1><p className="mt-1 text-sm text-gray-500">先构建主题的概念谱系和基础论文池，再从池中均匀随机抽取；没有稀有度和等级。</p></div>

    <div className="rounded-xl border border-gray-200 bg-white p-5">
      <div className="mb-4 flex items-center gap-2 font-semibold text-gray-900"><Search size={17} className="text-[#F97316]" />抽取设置 <span className="text-xs font-normal text-gray-400">仅从基础理论候选池中随机</span></div>
      <div className="space-y-4 text-sm">
        <div className="flex items-start gap-3"><span className="w-16 shrink-0 pt-2 text-gray-500">研究方向</span><div className="flex flex-1 flex-wrap gap-2">{topics.map(([label, value]) => <button key={value} onClick={() => { setQuery(value); setCustomQuery(''); setQueryInfo(null); }} className={`rounded-lg border px-4 py-2 ${!customQuery && query === value ? 'border-[#FDBA74] bg-[#FFF7ED] text-[#F97316]' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{label}</button>)}<input value={customQuery} onChange={(e) => { setCustomQuery(e.target.value); setQueryInfo(null); }} placeholder="自定义主题，例如 对称加密 / Anamorphic Encryption" className="min-w-[280px] flex-1 rounded-lg border border-gray-200 px-3 py-2 outline-none focus:border-[#FDBA74]" /></div><SearchLanguageToggle mode={searchLanguageMode} onChange={(mode) => { onSearchLanguageModeChange(mode); setQueryInfo(null); }} /></div>
        <div className="flex items-center gap-3"><span className="w-16 shrink-0 text-gray-500">时间范围</span><div className="flex flex-wrap items-center gap-2"><button onClick={() => setRangeMode('all')} className={`rounded-lg border px-4 py-2 ${rangeMode === 'all' ? 'border-[#FDBA74] bg-[#FFF7ED] text-[#F97316]' : 'border-gray-200 text-gray-600'}`}>全部年份</button><button onClick={() => setRangeMode('classic')} className={`rounded-lg border px-4 py-2 ${rangeMode === 'classic' ? 'border-[#FDBA74] bg-[#FFF7ED] text-[#F97316]' : 'border-gray-200 text-gray-600'}`}>经典基础</button><button onClick={() => setRangeMode('custom')} className={`rounded-lg border px-4 py-2 ${rangeMode === 'custom' ? 'border-[#FDBA74] bg-[#FFF7ED] text-[#F97316]' : 'border-gray-200 text-gray-600'}`}>自定义</button>{rangeMode === 'custom' && <div className="flex items-center gap-2"><input type="number" min={1800} max={2100} value={fromYear} onChange={(e) => setFromYear(Number(e.target.value))} className="w-24 rounded-lg border border-gray-200 px-2 py-2" /><span className="text-gray-300">—</span><input type="number" min={1800} max={2100} value={toYear} onChange={(e) => setToYear(Number(e.target.value))} className="w-24 rounded-lg border border-gray-200 px-2 py-2" /></div>}</div><label className="ml-auto flex items-center gap-2 text-gray-600"><span>基础论文优先</span><button onClick={() => setFoundationalOnly((v) => !v)} className={`relative h-6 w-11 rounded-full transition ${foundationalOnly ? 'bg-[#F97316]' : 'bg-gray-200'}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${foundationalOnly ? 'left-6' : 'left-1'}`} /></button></label></div>
      </div>
      <QueryLanguageNotice info={queryInfo} className="mt-4" />
    </div>
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-4">
      ...
      <div ref={viewportRef} className="relative overflow-hidden rounded-xl border border-gray-100 bg-[#FBFBFD] py-5">
        <div className="pointer-events-none absolute left-1/2 top-0 z-10 h-full w-px bg-[#F97316]"><span className="absolute -left-2 -top-1 border-x-[8px] border-t-[10px] border-x-transparent border-t-[#F97316]" /></div>
      ...
      <div className="mt-4 flex justify-center gap-3"><button disabled={loading || spinning} onClick={draw} className="flex min-w-44 items-center justify-center gap-2 rounded-lg bg-[#F97316] px-6 py-2.5 text-sm font-semibold text-white hover:bg-orange-600 disabled:opacity-50">{loading ? <Loader2 size={16} className="animate-spin" /> : <Dice5 size={16} />}{loading ? '正在构建基础池...' : spinning ? '正在抽取...' : selected ? '重新抽取' : '开始抽取'}</button>{selected && <button disabled={spinning} onClick={draw} className="flex items-center gap-2 rounded-lg border border-gray-200 px-5 py-2.5 text-sm text-gray-600"><RefreshCw size={15} />再抽一次</button>}</div>
...
function ReelCard({ item }: { item: GenealogyPaper }) {
  const p = item.paper;
  return <div className="h-40 w-44 shrink-0 overflow-hidden rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
    <div className="flex min-w-0 items-center gap-1 text-[11px] text-gray-400">
      <span className="shrink-0">{p.year} ·</span>
      <span className="min-w-0 flex-1 truncate" title={p.venue}>{p.venue}</span>
    </div>
    <div className="mt-2 line-clamp-4 break-words text-sm font-semibold leading-5 text-gray-900" title={p.titleEn}>{p.titleEn}</div>
    <div className="mt-3 truncate text-[11px] text-gray-400" title={p.authors.join(', ')}>{p.authors.join(', ')}</div>
  </div>;
}
...
function ResultCard({ item, scheduled, scheduleOpen, setScheduleOpen, weekDays, onSchedule, onSelectPaper, onAddReading, inReading }: { item: GenealogyPaper; scheduled: string; scheduleOpen: boolean; setScheduleOpen: (value: boolean) => void; weekDays: ReturnType<typeof getWeekDays>; onSchedule: (iso: string) => Promise<void>; onSelectPaper: (paper: Paper) => void; onAddReading: (paper: Paper) => Promise<void>; inReading: boolean }) {
  const p = item.paper;
  return <div><div className="flex items-start gap-4"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-[#FFF7ED] text-[#F97316]"><Dice5 size={24} /></div><div className="min-w-0 flex-1"><h2 className="text-lg font-bold leading-7 text-gray-900">{p.titleEn}</h2><div className="mt-1 truncate text-sm text-gray-500">{p.authors.join(', ')}</div><div className="mt-2 flex min-w-0 items-center gap-1 text-xs text-gray-400"><span className="shrink-0">{p.year} ·</span><span className="min-w-0 truncate" title={p.venue}>{p.venue}</span><span className="shrink-0">· {(p.citations || 0).toLocaleString()} 次被引</span></div></div></div><div className="mt-4 rounded-lg bg-[#FFF7ED] p-3 text-sm leading-6 text-gray-600"><span className="font-medium text-[#F97316]">为什么进入基础池：</span>{item.reason}</div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => onSelectPaper(p)} className="rounded-lg border border-[#FED7AA] px-4 py-2 text-sm font-medium text-[#F97316]">查看论文详情</button><button disabled={inReading} onClick={() => onAddReading(p)} className="flex items-center gap-2 rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-600 disabled:text-emerald-600"><BookOpenCheck size={15} />{inReading ? '已在阅读清单' : '加入阅读清单'}</button><div className="relative"><button onClick={() => setScheduleOpen(!scheduleOpen)} className="flex items-center gap-2 rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-600"><CalendarDays size={15} />{scheduled ? '已安排本周' : '安排本周 TODO'}</button>{scheduleOpen && <div className="absolute bottom-11 left-0 z-20 w-64 rounded-xl border border-gray-200 bg-white p-3 shadow-xl"><div className="mb-2 text-xs font-semibold text-gray-500">选择阅读日期</div><div className="grid grid-cols-7 gap-1">{weekDays.map((day) => <button key={day.iso} onClick={() => onSchedule(day.iso)} className="rounded-lg px-1 py-2 text-center text-[10px] hover:bg-[#FFF7ED] hover:text-[#F97316]"><span className="block font-medium">{day.weekday}</span><span className="mt-1 block text-gray-400">{day.shortDate}</span></button>)}</div></div>}</div><button onClick={() => onSelectPaper(p)} className="flex items-center gap-2 rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-600"><NotebookPen size={15} />写 Markdown 笔记</button>{scheduled && <span className="flex items-center gap-1 px-2 text-xs text-emerald-600"><Check size={14} />{scheduled}</span>}</div></div>;
}
