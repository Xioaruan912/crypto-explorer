  ...
  return <div className="space-y-4">
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <div className="space-y-4 text-sm">
        <div className="flex items-start gap-3"><span className="w-16 shrink-0 pt-2 text-gray-500">研究方向</span><div className="flex flex-1 flex-wrap gap-2">{topics.map(([label, value]) => <button key={value} onClick={() => { setQuery(value); setCustomQuery(''); setQueryInfo(null); }} className={`rounded-lg border px-4 py-2 ${!customQuery && query === value ? 'border-[#B8A8FF] bg-[#F7F5FF] text-[#6D4AFF]' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{label}</button>)}<input value={customQuery} onChange={(e) => { setCustomQuery(e.target.value); setQueryInfo(null); }} placeholder="自定义主题，例如 对称加密 / Anamorphic Encryption" className="min-w-[280px] flex-1 rounded-lg border border-gray-200 px-3 py-2 outline-none focus:border-[#B8A8FF]" /></div><SearchLanguageToggle mode={searchLanguageMode} onChange={(mode) => { onSearchLanguageModeChange(mode); setQueryInfo(null); }} /></div>
